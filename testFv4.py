...
cursor.execute("CREATE TABLE IF NOT EXISTS twitter_OD2 (Numero_mesure INT, Nombre_de_follower INT, Date2 time)")
Nmesure = 1
...
for i in range (5):
    ...
    print(final + ' ' + date + ' ' + heure)
    time.sleep(5)

    #cursor.execute("INSERT INTO twitter_OD2 VALUES ('"+ Nmesure +"')")
    cursor.execute("INSERT INTO twitter_OD2 VALUES (%s, %s, now() )" % (Nmesure, final))
